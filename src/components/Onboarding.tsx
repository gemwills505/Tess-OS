...
import { getBrain, updateBrain, saveAppSettings, applyAntiHeroStrategy } from '../services/brain';
...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const handleConfirmHire = async () => {
      ...
      // EXTRACT THE SITCOM ELEMENTS (Fallback to defaults if missing)
      const sitcom = selectedCandidate.sitcom_elements || {
          prop: "Coffee",
          mascot: "Pet",
          villain: "The Boss"
      };

      let newBrain: BrainData = {
          ...currentBrain,
          brandContext,
          personaCV,
          onboardingType: selectedArchetype,
      ...
      // *** THE MAGIC LINE: INJECT STRATEGY ***
      newBrain = applyAntiHeroStrategy(newBrain, sitcom);
  ...
  if (step === 'cv_review' && selectedCandidateId) {
      ...
      const currentMode = candidate.voiceModes?.[selectedVoiceModeIndex] || {
          title: candidate.label, mission: candidate.mission, voiceDescription: "Standard voice.", samplePost: "Check this out!", chaosLevel: candidate.chaosLevel, sarcasmLevel: