...
const FeedDock: React.FC = () => {
    ...
    const copyBriefForMilanote = (post: FeedPost) => {
        ...
        // Format specifically for a Milanote Note Card
    ...
    return (
        <div className="h-full flex flex-col bg-white w-full relative">
            <div className="flex border-b border-gray-200">
                ...
                <button onClick={() => setView('bank')} className={`flex-1 py-3 text-xs font-bold uppercase tracking-wider ${view === 'bank' ? 'text-brand-dark border-b-2 border-brand-dark' : 'text-gray-400 hover:text-gray-600'}`}>📂 Bank ({bankItems.length})</button>
            </div>
                                    ...
                                    {post.imageUrl ? (
                                        <>
                                            ...
                                            <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2">
                                                ...
                                                <button onClick={(e) => handleDeletePost(post.id, e)} className="text-[10px] bg-red-500 text-white px-2 py-1 rounded font-bold hover:bg-red-600">Clear</button>
                                            </div>
                                        </>
                                    ) : (
