...
// Declare globals locally to ensure no TS errors
declare global {
    interface Window {
        html2canvas: any;
        // aistudio is defined globally with type AIStudio
    }
}
...
const FeedPlanner: React.FC = () => {
  ...
  // Auto-save profile changes debounce (Except Avatar)
  useEffect(() => {
  ...
  const handleGenerateVeo = async () => {
      ...
      if (window.aistudio && !await window.aistudio.hasSelectedApiKey()) {
          try { await window.aistudio.openSelectKey(); } catch(e) { alert("Paid API Key required for Veo."); return; }
      }
  ...
  const handleDropOnFeed = (e: React.DragEvent, dropIndex: number) => {
    ...
    if (type === "feed") {
        ...
        // Remove from Bank if successfully dropped on feed
        if (bankId) {
            const currentBank = getBank();
  ...
  return (
    <div className="h-screen bg-gray-100 flex overflow-hidden font-sans relative">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 overflow-hidden relative">
          ...
          <div id="phone-mockup-content" className="w-full max-w-[400px] h-full max-h-[850px] bg-white rounded-[35px] shadow-2xl border-[8px] border-gray-100 flex flex-col overflow-hidden relative shrink-0">
               ...
               <div className="px-5 pt-4 pb-2 flex items-center justify-between bg-white z-10">
                    <div className="flex items-center gap-1 text-base font-bold text-gray-900 group relative"><input value={profileData.handle} onChange={(e) => handleProfileChange('handle', e.target.value)} className="bg-transparent border-b border-transparent hover:border-gray-300 focus:border-brand-purple outline-none max-w-[150px]" /><span>▼</span></div>
            ...
            <div className="flex-1 overflow-y-auto bg-white scroll-smooth scrollbar-hide">
                 ...
                 <div className="px-5 pt-2 pb-4">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        >
                             ...
                             <button className="absolute bottom-0 right-0 bg-blue-500 text-white rounded-full w-6 h-6 flex items-center justify-center shadow-md border-2 border-white transform hover:scale-110 transition-transform z-10 text-xs font-bold pointer-events-none">+</button>
                        </div>
                         <div className="flex flex-1 justify-around ml-4">
                            ...
                            <div className="text-center"><input value={profileData.followers} onChange={(e) => handleProfileChange('followers', e.target.value)} className="font-bold text-gray-900 text-sm w-12 text-center bg-transparent border-b border-transparent hover:border-gray-200 focus:border-brand-purple outline-none" /><div className="text-xs text-gray-900">Followers</div></div>
                            <div className="text-center"><input value={profileData.following} onChange={(e) => handleProfileChange('following', e.target.value)} className="font-bold text-gray-900 text-sm w-12 text-center bg-transparent border-b border-transparent hover:border-gray-200 focus:border-brand-purple outline-none" /><div className="text-xs text-gray-900">Following</div></div>
                        </div>
                    </div>

                    <div>
                        <input value={profileData.name} onChange={(e) => handleProfileChange('name', e.target.value)} className="font-bold text-sm text-gray-900 w-full bg-transparent border-none p-0 focus:ring-0 placeholder-gray-400" placeholder="Name" />
                        <input value={profileData.role} onChange={(e) => handleProfileChange('role', e.target.value)} className="text-sm text-gray-700 w-full bg-transparent border-none p-0 focus:ring-0 placeholder-transparent" placeholder="" />
                        <div className="relative group mt-1">
                            <textarea 
                                value={profileData.bio} 
                                onChange={(e) => handleProfileChange('bio', e.target.value)} 
                                className="text-sm text-gray-800 w-full bg-transparent border border-transparent hover:border-gray-200 rounded p-1 -ml-1 focus:border-brand-purple focus:ring-1 focus:ring-brand-purple/20 outline-none resize-none leading-snug min-h-[6rem] overflow-y-auto whitespace-pre-wrap" 
                                rows={5} 
                                maxLength={150}
                        ...
                        {highlights.map((hl) => (
                            ...
                            >
                                <div 
                                    ...
                                    title="Click to upload, Right-click to clear"
                                >
                                    {hl.coverUrl ? <img src={hl.coverUrl} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gray-100"></div>}
                                    <div className="absolute inset-0 bg-black/30 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity text-white font-bold">+</div>
                                </div>
                                <button 
                                    type="button"
                                    // onMouseDown is crucial here to stop the click event before it starts
                                    ...
                                    title={hl.coverUrl ? "Clear Image" : "Delete Highlight"}
                                >
                                    ✕
                                </button>
                                <input value={hl.name} onChange={(e) => handleHighlightNameChange(hl.id, e.target.value)} className="text-xs text-gray-900 text-center bg-transparent outline-none w-16 truncate focus:text-brand-purple font-medium" />
                            </div>
                        ))}
                         <button onClick={() => handleSaveHighlights([...highlights, { id: `HL_${Date.now()}`, name: 'New', coverUrl: null }])} className="flex flex-col items-center space-y-1 flex-shrink-0"><div className="w-16 h-16 rounded-full border border-dashed border-gray-300 flex items-center justify-center text-gray-400 hover:bg-gray-50 hover:border-brand-purple hover:text-brand-purple transition-all">+</div><span className="text-xs text-gray-400">Add</span></button>
                    </div>
                 </div>
                ...
                <div className="grid grid-cols-3 gap-[2px] bg-white pb-20">
                    {posts.map((post, index) => (
                        <div key={post.id} draggable onDragStart={(e) => handleFeedDragStart(e, index)} onDragOver={handleDragOver} onDrop={(e) => handleDropOnFeed(e, index)} onClick={() => handlePostClick(post)} className={`aspect-[4/5] relative group cursor-pointer transition-opacity ${draggedItemIndex === index ? 'opacity-40' : 'opacity-100'} ${selectedPostId === post.id ? 'ring-4 ring-inset ring-blue-500 z-10' : ''}`}>
                            {post.imageUrl || post.videoUrl ? (
                            ...
                            ) : post.type !== 'empty' && (post.caption || post.aiPrompt) ? (
                                ...
                                <div className="w-full h-full bg-gray-100 flex flex-col items-center justify-center text-gray-300 hover:bg-gray-200 transition-colors relative">{isAutoFilling && (post.type === 'empty' || !post.imageUrl) ? <span className="animate-pulse text-2xl">⚡️</span> : '+'}</div>
                            )}
                        </div>
                    ))}
                </div>
      ...
      <div className="w-[450px] bg-white border-l border-gray-200 flex flex-col shadow-xl z-20 relative">
          {selectedPostId ? (
             <>
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-white"><h3 className="font-bold text-lg text-gray-900">Edit Post</h3><button onClick={() => setSelectedPostId(null)} className="p-2 hover:bg-gray-100 rounded-full">✕</button></div>
                <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-hide">
                    {selectedPost && (
                        <>
                            <div className="relative aspect-[4/5] rounded-xl overflow-hidden group shadow-sm bg-gray-100">
                                {selectedPost.imageUrl ? <img src={selectedPost.imageUrl} className="w-full h-full object-cover" /> : selectedPost.videoUrl ? <video src={selectedPost.videoUrl} className="w-full h-full object-cover" controls /> : (
                                    <div className="w-full h-full bg-gray-50 flex flex-col items-center justify-center text-gray-400 p-6 text-center">
                                        ...
                                        {selectedPost.type === 'video' ? <p className="text-xs">Generate with Veo below.</p> : <p className="text-xs">Upload or generate image.</p>}
                                    </div>
                                )}
                                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                    <button onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-white rounded-lg text-sm font-bold shadow-lg">Change File</button>
                            ...
                            {selectedPost.type === 'image' && selectedPost.aiPrompt && (
                            ...
                            {selectedPost.type === 'video' || selectedPost.aiPrompt || selectedPost.imageUrl ? (
                            ...
                            <div className="space-y-2">
                                <div className="flex justify-between items-end"><label className="text-xs font-bold text-gray-500 uppercase tracking-wider">Caption (SEO Heavy)</label><button onClick={handleMagicCaption} disabled={loadingAi || !selectedPost.imageUrl} className={`text-xs font-bold px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition-all ${loadingAi ? 'bg-gray-100 text-gray-400' : 'bg-gradient-to-r from-purple-500 to-pink-500 text-white shadow-md'}`}>✨ Magic Write</button></div>
                                <textarea value={selectedPost.caption} onChange={(e) => handleUpdatePost('caption', e.target.value)} className="w-full h-40 p-4 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none resize-none bg-gray-50" placeholder="Write your caption here..." />
                            </div>
                             <div className="pt-4 border-t border-gray-100"><button onClick={handleDeletePost} className="w-full py-3 rounded-xl border border-red-100 text-red-500 font-bold text-sm hover:bg-red-50 transition-colors flex items-center justify-center gap-2">🗑️ Clear Slot</button></div>
                        </>
                    )}
                </div>
             </>
             ...
             <div className="flex flex-col h-full" onDragOver={handleDragOver} onDrop={handleDropOnBank}>
                 <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50"><h3 className="font-bold text-lg text-brand-dark tracking-tight">THUMBNAIL BANK</h3><button onClick={() => bankInputRef.current?.click()} className="p-2 bg-white border border-gray-200 rounded-lg hover:bg-gray-50 text-gray-600 shadow-sm">➕</button></div>
                <div className="flex-1 overflow-y-auto p-6 bg-gray-100/50 scrollbar-hide"><div className="grid grid-cols-3 gap-3 auto-rows-fr">{bankItems.map((item) => (<div key={item.id} draggable onDragStart={(e) => handleBankDragStart(e, item.id, item.imageUrl)} className="aspect-square rounded-lg overflow-hidden bg-white shadow-sm relative group cursor-grab active:cursor-grabbing hover:shadow-md transition-all"><img src={item.imageUrl} className="w-full h-full object-cover" draggable={false} /><button type="button" onClick={(e) => deleteFromBank(item.id, e)} onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); }} className="absolute top-1 right-1 w-6 h-6 bg-white/90 rounded-full flex items-center justify-center text-red-500 opacity-0 group-hover:opacity-100 transition-opacity shadow-md z-[100]">✕</button></div>))}</div></div>
             </div>
          )}
      </div>
    </div>
